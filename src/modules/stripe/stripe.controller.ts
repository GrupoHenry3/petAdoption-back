...
export class StripeController {
  ...
  private async handlePaymentIntentSucceeded(paymentIntent: Stripe.PaymentIntent) {
    ...
    try {
      ...
      const session = await this.stripeService.retrieveCheckoutSession(paymentIntent.metadata?.session_id);
      if (session) {
        await this.donationsService.markDonationAsCompleted(session.id);
  ...
  private async handlePaymentIntentFailed(paymentIntent: Stripe.PaymentIntent) {
    ...
    try {
      const session = await this.stripeService.retrieveCheckoutSession(paymentIntent.metadata?.session_id);
      if (session) {
        await this.donationsService.markDonationAsFailed(session.id, paymentIntent.last_payment_error?.message || 'Payment failed');
